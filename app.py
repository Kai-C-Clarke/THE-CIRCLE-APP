...
from ai_photo_matcher import (
    suggest_photos_for_memory,
    apply_suggestion,
    suggest_all_memories,
)
...
def save_profile():
    try:
        ...
        cursor.execute(
            """INSERT INTO user_profile (name, birth_date, family_role, birth_place, created_at) 
                         VALUES (?, ?, ?, ?, ?)""",
            (
                data["name"],
                data["birth_date"],
...
def save_memory():
    ...
    try:
        ...
        if not text:
            return (
                jsonify({"status": "error", "message": "Memory text is required"}),
                400,
            )
...
def delete_memory(memory_id):
    ...
    try:
        ...
        if audio_filename:
            ...
            cursor.execute(
                "DELETE FROM audio_transcriptions WHERE audio_filename = ?",
                (audio_filename,),
            )
            db.commit()
...
def get_memories():
    ...
    try:
        ...
        cursor = db.cursor()
        cursor.execute(
            """SELECT id, text, category, memory_date, year, 
...
def update_memory(memory_id):
    ...
    try:
        ...
        if date_input:
            ...
            cursor = db.execute(
                "SELECT year, memory_date FROM memories WHERE id = ?", (memory_id,)
            )
...
def get_photo_suggestions(memory_id):
    ...
    try:
        ...
        suggestions = suggest_photos_for_memory(
            memory_id, confidence_threshold=threshold
        )
...
def accept_photo_suggestion(memory_id):
    ...
    try:
        ...
        if not photo_id:
            return jsonify({"status": "error", "error": "photo_id required"}), 400
...
def suggest_all_photos():
    ...
    try:
        ...
        all_suggestions = suggest_all_memories(confidence_threshold=threshold)
...
# Debug endpoint - protected with authentication
...
if __name__ == "__main__":
    ...
    print(
        f"AI Search: {'ENABLED' if ai_searcher.client else 'DISABLED (set DEEPSEEK_API_KEY)'}"
    )
