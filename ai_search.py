...
class DeepSeekSearch:
    def __init__(self, api_key=None):
        ...
        if not self.api_key:
            print(
                "⚠️  Warning: No DeepSeek API key found. Search will use basic matching."
            )
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": """You are a family memory search assistant. 
                    ...
                    {
                        "role": "user",
                        "content": f"Analyze this question about family memories: {query}",
                    },
                ],
                temperature=0.1,
                max_tokens=500,
            ...
            try:
                ...
                response = self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": """You are a family memory expert. 
                ...
                return {
                    "answer": result.get(
                        "answer",
                        "I couldn't find that information in the family memories.",
                    ),
    ...
    def _prepare_memory_context(self, memories: List[Dict]) -> str:
        ...
        return "\n\n".join(
            context_lines[:30]
        )  # Limit to 30 memories to avoid token limit
    ...
    def _enhanced_search(self, query: str, memories: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            "answer": answer
            or f"I found {len(scored_memories)} relevant memories, but couldn't extract a direct answer. Try looking at the memories below.",
