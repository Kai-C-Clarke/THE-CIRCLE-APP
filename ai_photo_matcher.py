...
def score_photo_match(memory_text, memory_year, photo_metadata):
    ...
    if memory_year and photo_year:
        try:
            ...
            # Years not comparable (None, invalid format, etc.)
            pass
    ...
    if matched_names:
        strong_signals += 1
    if visual_descriptions and any(
        w in photo_text
        for desc in visual_descriptions
        for w in desc.split()
        if len(w) > 4
    ...
    return {"score": min(100, score), "reasons": reasons}  # Cap at 100


def suggest_photos_for_memory(
    memory_id, db_path="circle_memories.db", confidence_threshold=40
):
    """
    ...
    for photo in photos:
        ...
        # Score the match
        metadata = {"title": title or "", "description": desc or "", "year": year}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced text-based photo matching for memories"
    )
    parser.add_argument(
        "--memory", type=int, help="Suggest photos for specific memory ID"
    )
    parser.add_argument(
        "--threshold", type=int, default=40, help="Score threshold (0-100)"
    )
    parser.add_argument(
        "--database", default="circle_memories.db", help="Database path"
    )
    parser.add_argument(
        "--apply",
        nargs=2,
        type=int,
        metavar=("MEMORY_ID", "PHOTO_ID"),
    ...
    parser.add_argument(
        "--test", action="store_true", help="Test visual description extraction"
    )
    ...
    if args.test:
        ...
        suggestions = suggest_photos_for_memory(
            args.memory, args.database, args.threshold
        )
